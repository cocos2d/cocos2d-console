...
class CCPluginCompile(cocos.CCPlugin):
    ...
    def _add_custom_options(self, parser):
        ...
        group = parser.add_argument_group("Tizen Options")
        group.add_argument("--tizen-arch", dest="tizen_arch", choices=[ "x86", "arm" ], help="Determines the architecture type for the rootstrap.")
        group.add_argument("--tizen-compiler", dest="tizen_compiler", choices=[ "llvm", "gcc" ], help="Set the compiler to the native project.")
        group.add_argument("--tizen-pkgtype", dest="tizen_pkgtype", choices=[ "tpk", "wgt" ], help="Set the package type..")
        group.add_argument("--tizen-profile", dest="tizen_profile", help="Set the profile path for signing.")
        group.add_argument("--tizen-sign", dest="tizen_sign", help="Set the profile name to use for signing.")
        group.add_argument("--tizen-strip", dest="tizen_strip", action="store_true", help="Determines whether to strip the native binary.")
    ...
    def build_tizen(self):
        ...
        if self.tizen_profile is None:
            raise cocos.CCPluginError("Please specify the profile path by --tizen-profile")
        ...
        if not os.path.isdir(build_cfg_path):
            raise cocos.CCPluginError("%s is not generated by 'tizen build-native'" % build_cfg_path)

        package_cmd = "%s package -- \"%s\"" % (tizen_cmd_path, build_cfg_path)
        if self.tizen_pkgtype is None:
            raise cocos.CCPluginError("Please specify the package type by --tizen-pkgtype.")
        else:
        ...
        if not os.path.isfile(tizen_pkg_path):
            raise cocos.CCPluginError("Build tizen package failed!")
